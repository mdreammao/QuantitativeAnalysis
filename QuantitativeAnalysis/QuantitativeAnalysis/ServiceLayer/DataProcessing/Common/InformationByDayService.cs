...
{
    public abstract class InformationByDayService<T> where T: new ()
    {
        ...
        /// <returns></returns>
        public abstract List<T> readFromWindOnly(string code, DateTime date, string tag = null, IDictionary<string, object> options = null);
        ...
        public List<T> fetchFromWind(string code, DateTime date, string tag = null)
        {
            ...
            try
            {
                result = readFromWindOnly(code, date,null, null);
            }
            catch (Exception e)
            {
